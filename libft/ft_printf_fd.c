...
/*   Updated: 2025/05/14 21:27:59 by alpayet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	process_arg(int fd, const char *format, size_t index, va_list ap)
{
	...
	if (format[index] == 'u')
		return (ft_putnbr_base_fd(va_arg(ap, unsigned int), BASE_DECIMAL, fd));
	if (format[index] == 'x')
		return (ft_putnbr_base_fd(va_arg(ap, unsigned int), BASE_SMALL_HEX, fd));
	if (format[index] == 'X')
		return (ft_putnbr_base_fd(va_arg(ap, unsigned int), BASE_BIG_HEX, fd));
	return (0);
}
