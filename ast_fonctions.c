...
/*   Updated: 2025/03/20 13:07:55 by alpayet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_AST_node	*create_parent_node(t_control_operator ope, t_AST_node *left, t_AST_node *right)
{
	t_AST_node	*node;
...
t_AST_node	*create_ast(t_leaf *command_tab)
{
	...
	search_latest_op(PIPE, command_tab, &buff);
	if (buff != NULL)
	{
		op = buff->ope_after;
		buff->ope_after = VOID;
